...
- Sites And Pool Profiles: service locations, one or more pools/spas/water bodies, pool volume, type, surface, sanitiser, indoor/outdoor, heaters, access instructions, warnings, and technician notes. Fixed target chemistry should not be stored on the pool profile; water testing should show guide ranges based on pool and equipment context.
...
- Chemical Guidance: BioGuard Australia first, with room for Pool Pro and generic products later. The MVP should suggest review-required product categories and possible products from readings, guide ranges, product purpose, application methods, safety notes, alternatives, and warnings. Exact dosing automation should wait until product rules are verified.
