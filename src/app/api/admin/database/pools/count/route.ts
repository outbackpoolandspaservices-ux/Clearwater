...
export async function GET(request: NextRequest) {
  if (!isAuthorised(request)) {
    return NextResponse.json(
      { ok: false, message: "Unauthorized pool count request." },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      dataSourceMode,
      databaseUrlConfigured: true,
      pools: result[0]?.count ?? 0,
      tableName: "pools",
    });
  } catch (error) {
    console.error("Pool count verification failed", error);

    const errorName = error instanceof Error ? error.name : "UnknownError";
    const errorMessage =
      error instanceof Error
        ? error.message
        : "Unknown pool count verification error.";
