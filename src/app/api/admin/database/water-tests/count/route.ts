...
export async function GET(request: NextRequest) {
  if (!isAuthorised(request)) {
    return NextResponse.json(
      { ok: false, message: "Unauthorized water test count request." },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      dataSourceMode,
      databaseUrlConfigured: true,
      tableName: "water_tests",
      waterTests: result[0]?.count ?? 0,
    });
  } catch (error) {
    console.error("Water test count verification failed", error);

    const errorName = error instanceof Error ? error.name : "UnknownError";
    const errorMessage =
      error instanceof Error
        ? error.message
        : "Unknown water test count verification error.";

    return NextResponse.json(
      {
        ok: false,
        stage: "water-test-count",
        errorName,
        errorMessage,
        dataSourceMode,
        databaseUrlConfigured: true,
