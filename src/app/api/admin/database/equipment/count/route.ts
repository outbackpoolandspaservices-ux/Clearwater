...
export async function GET(request: NextRequest) {
  if (!isAuthorised(request)) {
    return NextResponse.json(
      { ok: false, message: "Unauthorized equipment count request." },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      dataSourceMode,
      databaseUrlConfigured: true,
      equipment: result[0]?.count ?? 0,
      tableName: "equipment",
    });
  } catch (error) {
    console.error("Equipment count verification failed", error);

    const errorName = error instanceof Error ? error.name : "UnknownError";
    const errorMessage =
      error instanceof Error
        ? error.message
        : "Unknown equipment count verification error.";

    return NextResponse.json(
      {
        ok: false,
        stage: "equipment-count",
        errorName,
        errorMessage,
        dataSourceMode,
        databaseUrlConfigured: true,
