...
function safeErrorSummary(error: unknown) {
  return {
    ...
    errorMessage:
      error instanceof Error ? error.message : "Unknown invoice save error.",
  };
}
...
export async function createInvoiceAction(
  ...
  const fieldErrors: NonNullable<CreateInvoiceFormState["fieldErrors"]> = {};
  ...
  if (!hasDatabaseUrl()) {
    return { formError: "Invoice creation needs DATABASE_URL configured." };
  }
  ...
  try {
    if (!(await tableExists(client, "invoices"))) {
      return {
        formError:
          "The invoices table is not available yet. Please run database setup.",
      };
    }
    ...
    const candidateValues: Record<string, Date | number | string | null> = {
      ...
      issued_at: new Date(),
      due_at: dueAt ? new Date(`${dueAt}T17:00:00`) : null,
    ...
    if (await tableExists(client, "invoice_line_items")) {
      const itemColumns = await getTableColumns(client, "invoice_line_items");
      ...
      await client.unsafe(
        `insert into "invoice_line_items" (${lineColumns
          .map(quoteIdentifier)
          .join(", ")})
    ...
    console.error("Invoice creation failed", {
      ...
      formFields: { customerId, jobId, quoteId, siteId },
      error,
    });

    return {
      formError:
        "ClearWater could not save this invoice. Please check the server logs for the safe summary.",
    };
  } finally {
    await client.end();
  }

  redirect("/invoices?created=invoice");
}
